...
`opencode-auth-login` 会启动 OpenCode 的交互式模型服务商认证流程。先选择要使用
的 Provider，再按照该 Provider 的提示完成 OAuth 登录或输入 API Key。该流程与
[OpenCode Providers 官方文档](https://opencode.ai/docs/providers)记录的各服务商
认证方式一致。
